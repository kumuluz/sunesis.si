...
export function SubNavbar({nav}) {
    ...
    return (
        <div className="subheader position-relative d-flex">
            ...
            <div
                className={'d-flex align-items-center justify-content-center w-100 ' + (bigEnough ? 'mx-auto' : '')}>
                {leftArray.map((prod, i) => (
                    <div key={i} className="text-center">
                        <Link to={prod.href} external={false}
                            className={isActive(prod) ? (prod.id ? prod.id : 'active') : ''}>
                            {t(prod.name)}
                        </Link>
                    </div>
                ))}
