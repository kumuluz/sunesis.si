...
export function KumuluzSection({
  ...
  return (
    <Section bg="dark" id="kumuluz">
      ...
      <RevealGroup className="mt-12 flex flex-wrap gap-4">
        {block.items.map((item) => (
          <motion.article
            className="grow basis-full rounded-lg border border-white/10 bg-white/[0.04] p-5 transition-colors duration-200 hover:bg-neutral-900 md:basis-[calc((100%-1rem)/2)] xl:basis-[calc((100%-2rem)/3)]"
            key={item.title}
